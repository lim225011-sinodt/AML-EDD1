...
-- AML300数据库模拟数据生成脚本
-- 基于300号文件接口规范
-- 数据规模: 1000个人客户 + 100个企业客户 + 10000笔交易
-- 创建时间: 2025-11-11
-- 版本: v1.0
-- =====================================================
...
-- 1. 银行网点数据 (20个网点)
-- =====================================================
INSERT INTO tb_bank (Bank_code1, Bank_name, Bank_type, Addr, Phone, Manager) VALUES
('104100000019', '中国农业银行北京中关村支行', '1', '北京市海淀区中关村大街27号', '010-62551188', '张明'),
('104100000020', '中国农业银行北京金融街支行', '1', '北京市西城区金融大街33号', '010-66081188', '李华'),
('104100000021', '中国农业银行北京国贸支行', '1', '北京市朝阳区建国门外大街甲14号', '010-65051188', '王强'),
('104100000022', '中国农业银行北京西单支行', '1', '北京市西城区西单北大街120号', '010-66018888', '赵明'),
('104310000001', '中国农业银行上海浦东支行', '1', '上海市浦东新区世纪大道100号', '021-58881188', '陈静'),
('104310000002', '中国农业银行上海黄浦支行', '1', '上海市黄浦区南京东路99号', '021-63218888', '刘伟'),
('104310000003', '中国农业银行上海徐汇支行', '1', '上海市徐汇区肇嘉浜路808号', '021-64381188', '周涛'),
('104440300001', '中国农业银行深圳南山支行', '1', '深圳市南山区深南大道9688号', '0755-26661188', '杨帆'),
('104440300002', '中国农业银行深圳福田支行', '1', '深圳市福田区深南大道7028号', '0755-83681188', '黄丽'),
('104440300003', '中国农业银行深圳罗湖支行', '1', '深圳市罗湖区深南东路5033号', '0755-82381188', '林峰'),
('104330100001', '中国农业银行杭州西湖支行', '1', '杭州市西湖区文三路90号', '0571-87901188', '张磊'),
('104330100002', '中国农业银行杭州滨江支行', '1', '杭州市滨江区江南大道3888号', '0571-86681188', '王芳'),
('104510100001', '中国农业银行成都锦江支行', '1', '成都市锦江区红星路三段1号', '028-86651188', '李敏'),
('104510100002', '中国农业银行成都武侯支行', '1', '成都市武侯区人民南路四段11号', '028-85551188', '陈亮'),
('104420100001', '中国农业银行广州天河支行', '1', '广州市天河区天河路123号', '020-87551188', '张勇'),
('104420100002', '中国农业银行广州越秀支行', '1', '广州市越秀区中山三路11号', '020-83331188', '刘洋'),
('104320100001', '中国农业银行南京鼓楼支行', '1', '南京市鼓楼区中山北路1号', '025-83281188', '王刚'),
('104320100002', '中国农业银行南京秦淮支行', '1', '南京市秦淮区洪武路299号', '025-84681188', '赵婷'),
('104120100001', '中国农业银行西安雁塔支行', '1', '西安市雁塔区小寨西路196号', '029-85218888', '李雪'),
('104120100002', '中国农业银行西安碑林支行', '1', '西安市碑林区南大街8号', '029-87218888', '陈阳');

-- =====================================================
-- 2. 个人客户数据 (1000个个人客户)
-- =====================================================
-- 生成个人客户基础数据
INSERT INTO tb_cst_pers (Cst_no, Acc_name, Sex, Birth_dt, Id_type, Id_no, Nation, Occ_type, Degree, Phone, Mobile, Email, Reg_addr, Live_addr, Work_unit)
SELECT
    CONCAT('P', LPAD(seq, 6, '0')) as Cst_no,
    CASE
        WHEN seq <= 100 THEN '张' || FLOOR(RAND() * 1000)
        WHEN seq <= 200 THEN '李' || FLOOR(RAND() * 1000)
        WHEN seq <= 300 THEN '王' || FLOOR(RAND() * 1000)
        WHEN seq <= 400 THEN '刘' || FLOOR(RAND() * 1000)
        WHEN seq <= 500 THEN '陈' || FLOOR(RAND() * 1000)
        WHEN seq <= 600 THEN '杨' || FLOOR(RAND() * 1000)
        WHEN seq <= 700 THEN '黄' || FLOOR(RAND() * 1000)
        WHEN seq <= 800 THEN '赵' || FLOOR(RAND() * 1000)
        WHEN seq <= 900 THEN '周' || FLOOR(RAND() * 1000)
        ELSE '吴' || FLOOR(RAND() * 1000)
    END as Acc_name,
    CASE WHEN RAND() > 0.5 THEN '1' ELSE '2' END as Sex,
    CONCAT(1970 + FLOOR(RAND() * 50), LPAD(1 + FLOOR(RAND() * 12), 2, '0'), LPAD(1 + FLOOR(RAND() * 28), 2, '0')) as Birth_dt,
    CASE WHEN RAND() > 0.1 THEN '11' ELSE '12' END as Id_type,
    CASE WHEN RAND() > 0.1 THEN
        -- 18位身份证
        CONCAT(
            CASE FLOOR(RAND() * 3)
                WHEN 0 THEN '110' -- 北京
            ...
            LPAD(FLOOR(RAND() * 100), 3, '0'),
            LPAD(1970 + FLOOR(RAND() * 50), 4, '0'),
            LPAD(1 + FLOOR(RAND() * 12), 2, '0'),
            LPAD(1 + FLOOR(RAND() * 28), 2, '0'),
            LPAD(FLOOR(RAND() * 1000), 3, '0'),
            CASE FLOOR(RAND() * 10) WHEN 0 THEN 'X' ELSE LPAD(FLOOR(RAND() * 10), 1, '0') END
        )
    ELSE
        -- 15位身份证
        CONCAT(
    ...
    CASE FLOOR(RAND() * 20)
        ...
        WHEN 1 THEN '专业技术人员'
        WHEN 2 THEN '办事人员和有关人员'
        WHEN 3 THEN '商业、服务业人员'
        WHEN 4 THEN '农、林、牧、渔、水利业生产人员'
        WHEN 5 THEN '生产、运输设备操作人员及有关人员'
        WHEN 6 THEN '军人'
        WHEN 7 THEN '不便分类的其他从业人员'
        ELSE '商业、服务业人员'
    END as Occ_type,
    CASE FLOOR(RAND() * 5)
        WHEN 0 THEN '研究生'
        ...
        ELSE '初中'
    END as Degree,
    CONCAT('0', FLOOR(RAND() * 9000000000 + 1000000000)) as Phone,
    CONCAT('1', FLOOR(RAND() * 9000000000 + 3000000000)) as Mobile,
    CONCAT('user', seq, '@example.com') as Email,
    CONCAT('北京市海淀区', FLOOR(RAND() * 100 + 1), '号') as Reg_addr,
    CONCAT('北京市朝阳区', FLOOR(RAND() * 100 + 1), '号') as Live_addr,
    CASE FLOOR(RAND() * 10)
        WHEN 0 THEN '中国银行'
        WHEN 1 THEN '工商银行'
        WHEN 2 THEN '建设银行'
...
-- 3. 企业客户数据 (100个企业客户)
-- =====================================================
INSERT INTO tb_cst_unit (Cst_no, Acc_name, Legal_name, Legal_id_no, Legal_phone, Reg_capital, Est_dt, License, Org_code, Tax_no, Bus_scope, Reg_addr, Work_addr, Phone, Email, Rep_name, Rep_phone)
SELECT
    CONCAT('U', LPAD(seq, 6, '0')) as Cst_no,
    CASE
        WHEN seq <= 20 THEN
            CASE FLOOR(RAND() * 5)
                WHEN 0 THEN '北京' || FLOOR(RAND() * 100 + 1) || '科技有限公司'
                WHEN 1 THEN '上海' || FLOOR(RAND() * 100 + 1) || '信息技术有限公司'
                WHEN 2 THEN '深圳' || FLOOR(RAND() * 100 + 1) || '网络科技有限公司'
                WHEN 3 THEN '广州' || FLOOR(RAND() * 100 + 1) || '电子商务有限公司'
                ELSE '杭州' || FLOOR(RAND() * 100 + 1) || '软件技术有限公司'
            END
        WHEN seq <= 40 THEN
            CASE FLOOR(RAND() * 5)
                WHEN 0 THEN '北京' || FLOOR(RAND() * 100 + 1) || '贸易有限公司'
                WHEN 1 THEN '上海' || FLOOR(RAND() * 100 + 1) || '进出口有限公司'
                WHEN 2 THEN '深圳' || FLOOR(RAND() * 100 + 1) || '供应链管理有限公司'
                WHEN 3 THEN '广州' || FLOOR(RAND() * 100 + 1) || '物流有限公司'
                ELSE '杭州' || FLOOR(RAND() * 100 + 1) || '商贸有限公司'
            END
        WHEN seq <= 60 THEN
            CASE FLOOR(RAND() * 5)
                WHEN 0 THEN '北京' || FLOOR(RAND() * 100 + 1) || '投资管理有限公司'
                WHEN 1 THEN '上海' || FLOOR(RAND() * 100 + 1) || '资产管理有限公司'
                WHEN 2 THEN '深圳' || FLOOR(RAND() * 100 + 1) || '金融信息服务有限公司'
                WHEN 3 THEN '广州' || FLOOR(RAND() * 100 + 1) || '基金管理有限公司'
                ELSE '杭州' || FLOOR(RAND() * 100 + 1) || '股权投资有限公司'
            END
        WHEN seq <= 80 THEN
            CASE FLOOR(RAND() * 5)
                WHEN 0 THEN '北京' || FLOOR(RAND() * 100 + 1) || '制造有限公司'
                WHEN 1 THEN '上海' || FLOOR(RAND() * 100 + 1) || '实业有限公司'
                WHEN 2 THEN '深圳' || FLOOR(RAND() * 100 + 1) || '电子科技有限公司'
                WHEN 3 THEN '广州' || FLOOR(RAND() * 100 + 1) || '机械有限公司'
                ELSE '杭州' || FLOOR(RAND() * 100 + 1) || '自动化有限公司'
            END
        ELSE
            CASE FLOOR(RAND() * 5)
                WHEN 0 THEN '北京' || FLOOR(RAND() * 100 + 1) || '咨询服务有限公司'
                WHEN 1 THEN '上海' || FLOOR(RAND() * 100 + 1) || '企业管理有限公司'
                WHEN 2 THEN '深圳' || FLOOR(RAND() * 100 + 1) || '文化创意有限公司'
                WHEN 3 THEN '广州' || FLOOR(RAND() * 100 + 1) || '广告有限公司'
                ELSE '杭州' || FLOOR(RAND() * 100 + 1) || '设计有限公司'
            END
    END as Acc_name,
    CASE FLOOR(RAND() * 20)
        WHEN 0 THEN '张伟'
    ...
    CONCAT('138', LPAD(FLOOR(RAND() * 100000000), 8, '0')) as Legal_phone,
    (FLOOR(RAND() * 900 + 100) * 10000) as Reg_capital,
    CONCAT(2000 + FLOOR(RAND() * 25), LPAD(1 + FLOOR(RAND() * 12), 2, '0'), LPAD(1 + FLOOR(RAND() * 28), 2, '0')) as Est_dt,
    CONCAT(
        '91',
        CASE FLOOR(RAND() * 5)
            WHEN 0 THEN '110000' -- 北京
            WHEN 1 THEN '310000' -- 上海
            WHEN 2 THEN '440300' -- 深圳
            WHEN 3 THEN '330100' -- 杭州
            ELSE '510100' -- 成都
        END,
        ...
        CASE FLOOR(RAND() * 35)
            ...
            WHEN 10 THEN 'A' WHEN 11 THEN 'B' WHEN 12 THEN 'C' WHEN 13 THEN 'D' WHEN 14 THEN 'E'
            WHEN 15 THEN 'F' WHEN 16 THEN 'G' WHEN 17 THEN 'H' WHEN 18 THEN 'J' WHEN 19 THEN 'K'
            WHEN 20 THEN 'L' WHEN 21 THEN 'M' WHEN 22 THEN 'N' WHEN 23 THEN 'P' WHEN 24 THEN 'Q'
            WHEN 25 THEN 'R' WHEN 26 THEN 'T' WHEN 27 THEN 'U' WHEN 28 THEN 'W' WHEN 29 THEN 'X'
            WHEN 30 THEN 'Y' WHEN 31 THEN 'Z' WHEN 32 THEN '2' WHEN 33 THEN '3' WHEN 34 THEN '4'
        END
    ) as License,
    CONCAT(LPAD(FLOOR(RAND() * 10000000), 8, '0'), '-', LPAD(FLOOR(RAND() * 10), 1, '0')) as Org_code,
    LPAD(FLOOR(RAND() * 1000000000), 11, '0') as Tax_no,
    CASE FLOOR(RAND() * 10)
        WHEN 0 THEN '软件开发；技术服务；技术咨询；技术转让；计算机系统集成'
        WHEN 1 THEN '国内贸易；进出口业务；供应链管理；物流服务'
        WHEN 2 THEN '投资管理；资产管理；股权投资；创业投资'
        WHEN 3 THEN '生产制造；加工组装；销售服务；技术支持'
        WHEN 4 THEN '企业管理咨询；商务咨询；市场营销策划'
        WHEN 5 THEN '互联网信息服务；数据处理服务；信息技术服务'
        WHEN 6 THEN '电子产品销售；通信设备销售；办公用品销售'
        WHEN 7 THEN '机械设备销售；自动化设备销售；工业设备销售'
        WHEN 8 THEN '广告设计；品牌策划；创意设计；文化活动策划'
        ELSE '技术进出口；货物进出口；代理进出口；贸易代理'
    END as Bus_scope,
    CASE FLOOR(RAND() * 5)
        WHEN 0 THEN '北京市海淀区中关村大街' || FLOOR(RAND() * 1000 + 1) || '号'
        WHEN 1 THEN '上海市浦东新区陆家嘴环路' || FLOOR(RAND() * 1000 + 1) || '号'
        WHEN 2 THEN '深圳市南山区科技园路' || FLOOR(RAND() * 1000 + 1) || '号'
        WHEN 3 THEN '广州市天河区珠江新城' || FLOOR(RAND() * 1000 + 1) || '号'
        ELSE '杭州市西湖区文三路' || FLOOR(RAND() * 1000 + 1) || '号'
    END as Reg_addr,
    CASE FLOOR(RAND() * 5)
        WHEN 0 THEN '北京市朝阳区建国门外大街' || FLOOR(RAND() * 1000 + 1) || '号'
        WHEN 1 THEN '上海市黄浦区南京东路' || FLOOR(RAND() * 1000 + 1) || '号'
        WHEN 2 THEN '深圳市福田区深南大道' || FLOOR(RAND() * 1000 + 1) || '号'
        WHEN 3 THEN '广州市越秀区中山三路' || FLOOR(RAND() * 1000 + 1) || '号'
        ELSE '杭州市上城区解放路' || FLOOR(RAND() * 1000 + 1) || '号'
    END as Work_addr,
    CONCAT('010', LPAD(FLOOR(RAND() * 100000000), 8, '0')) as Phone,
    CONCAT('company', seq, '@example.com') as Email,
    CASE FLOOR(RAND() * 20)
        WHEN 0 THEN '张经理'
        WHEN 1 THEN '王经理'
        WHEN 2 THEN '李经理'
        ...
        ELSE '郑经理'
    END as Rep_name,
    CONCAT('139', LPAD(FLOOR(RAND() * 100000000), 8, '0')) as Rep_phone
FROM (
    SELECT 1 as seq UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION
    SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10 UNION
    SELECT 11 UNION SELECT 12 UNION SELECT 13 UNION SELECT 14 UNION SELECT 15 UNION
...
SELECT '基础客户数据生成完成！继续生成其他表数据...' AS Message;